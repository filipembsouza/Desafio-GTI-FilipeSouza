...
public interface VincPerfilFuncionalidadeRepository extends JpaRepository<VincPerfilFuncionalidade, Long> {
    ...
    /**
     * Adiciona um conjunto de funcionalidades a um perfil.
     * Facilita a atribuição em massa de permissões.
     * 
     * @param perfilId ID do perfil a receber as permissões
     * @param funcionalidadeIds IDs das funcionalidades a serem vinculadas
     * @return O número de vinculos criados
     */
    @Modifying
    @Transactional
    @Query(value = "INSERT INTO vinc_perfil_funcionalidade (id_perfil, funcionalidade_id) " +
                  "SELECT :perfilId, f.id FROM funcionalidade f WHERE f.id IN :funcionalidadeIds " +
                  "AND NOT EXISTS (SELECT 1 FROM vinc_perfil_funcionalidade v WHERE v.id_perfil = :perfilId AND v.funcionalidade_id = f.id)",
           nativeQuery = true)
    int addFuncionalidadesToPerfil(
            @Param("perfilId") Long perfilId,
            @Param("funcionalidadeIds") Set<Long> funcionalidadeIds);
}